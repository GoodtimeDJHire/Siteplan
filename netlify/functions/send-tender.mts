...
async function timedFetch(url:string,init:RequestInit={},timeoutMs=12000){const controller=new AbortController();const timer=setTimeout(()=>controller.abort(),timeoutMs);try{return await fetch(url,{...init,signal:controller.signal})}catch(error:any){if(error?.name==="AbortError")throw new Error("The upstream service timed out. Please try again.");throw error}finally{clearTimeout(timer)}}
async function supabaseGet(path:string,jwt:string){const r=await timedFetch(`${SUPABASE_URL}/rest/v1/${path}`,{headers:{apikey:SUPABASE_KEY,Authorization:`Bearer ${jwt}`,Accept:"application/json"}});if(!r.ok)throw new Error(`Database request failed (${r.status})`);return r.json()}
async function supabaseInsert(path:string,rows:any[],jwt:string){const r=await timedFetch(`${SUPABASE_URL}/rest/v1/${path}`,{method:"POST",headers:{apikey:SUPABASE_KEY,Authorization:`Bearer ${jwt}`,"Content-Type":"application/json",Prefer:"return=minimal"},body:JSON.stringify(rows)});if(!r.ok)throw new Error(`Database insert failed (${r.status}): ${await r.text()}`)}
...
export default async(req:Request,context:Context)=>{
 ...
 try{
  ...
  const ur=await timedFetch(`${SUPABASE_URL}/auth/v1/user`,{headers:{apikey:SUPABASE_KEY,Authorization:`Bearer ${jwt}`}});if(!ur.ok)return Response.json({error:"Your SitePlan session has expired. Please sign in again."},{status:401});const user=await ur.json();
  ...
  const suppliers=await supabaseGet("suppliers?select=*&active=eq.true",jwt),matched=(suppliers||[]).filter((s:any)=>s.email&&s.notifications!=="off"&&categoryMatches(s,t)&&regionMatches(s,t)&&(s.notifications!=="high"||score(s,t)>=85));const selected=new Set(ids),chosen=matched.filter((s:any)=>selected.has(String(s.id))),recipients=[...new Map(chosen.map((s:any)=>[String(s.email).trim().toLowerCase(),s])).values()];if(!recipients.length)return Response.json({error:"None of the selected suppliers are eligible matching recipients with an email address."},{status:400});
  ...
  const requestId=String(body.requestId||crypto.randomUUID()).replace(/[^a-zA-Z0-9_-]/g,"").slice(0,80)||crypto.randomUUID();
  const sr=await timedFetch("https://api.resend.com/emails/batch",{method:"POST",headers:{Authorization:`Bearer ${apiKey}`,"Content-Type":"application/json","Idempotency-Key":`siteplan-${t.id}-${requestId}`},body:JSON.stringify(emails)},20000),sd=await sr.json().catch(()=>({}));if(!sr.ok){console.error("Resend error",sd);return Response.json({error:sd?.message||"Email delivery request failed."},{status:502})}
  const rr=Array.isArray(sd?.data)?sd.data:[],now=new Date().toISOString(),tracking=recipients.map((s:any,i:number)=>({tender_id:t.id,owner_id:user.id,supplier_id:s.id||null,company_name:s.company_name||"Supplier",recipient_email:s.email,resend_email_id:rr[i]?.id||null,status:"sent",email_type:"tender_invitation",sent_at:now,updated_at:now}));
  context.waitUntil(supabaseInsert("tender_email_deliveries",tracking,jwt).catch(e=>console.error("Tender delivery tracking insert failed",e)));
  return Response.json({ok:true,sent:recipients.length,requestId,recipients:recipients.map((s:any,i:number)=>({company:s.company_name,email:s.email,deliveryId:rr[i]?.id||null}))});
