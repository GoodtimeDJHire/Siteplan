// SitePlan V85: canonical tender editor + editable tender email composer
(()=>{
'use strict';
...
window.shareTenderToAll=async function(id){
 ...
 bg.innerHTML=`<div class="sp-send"><div class="sp-head"><div class="sp-icon">✉</div><div><div class="sp-title">Send tender to suppliers</div><div class="sp-sub">Choose recipients and edit the email before it is sent.</div></div><button class="sp-x">×</button></div><div class="sp-body"><div class="sp-box"><b>${escHtml(t.title||'Tender')}</b><div class="sp-meta">Event: ${escHtml(eventName)} &nbsp; | &nbsp; Category: ${escHtml(t.category||'Supplier')} &nbsp; | &nbsp; Response due: ${escHtml(tenderDue(t))}</div></div><div class="sp-label">Matched suppliers (${matches.length})</div><div class="sp-note">Untick anyone you don't want to receive this tender.</div><div class="sp-list">${matches.map(s=>`<label class="sp-row"><input class="sp-pick" type="checkbox" value="${escHtml(s.id)}" checked><strong>${escHtml(s.company)}</strong><span class="sp-muted">${escHtml(s.email)}</span><span class="sp-muted hide-mobile">${escHtml((s.categories||[]).join(', '))}</span></label>`).join('')}</div><div class="sp-label">Email</div><div class="sp-note">Edit this for this send. The placeholders below are personalised for each supplier.</div><div class="sp-mail-fields"><div><label>Subject</label><input id="spTenderSubject" value="${escHtml(`[Event] – [Tender]`)}"></div><div><label>Message</label><textarea id="spTenderMessage">${escHtml(defaultMessage(t,eventName))}</textarea><div class="sp-template-help">Available placeholders: [Supplier], [Organiser], [Tender], [Event], [Category], [Region], [Due Date], [Tender Link]</div></div></div><div class="sp-actions"><button class="sp-btn cancel">Cancel</button><button class="sp-btn primary send">Send tender</button></div></div></div>`;
 ...
 bg.querySelector('.send').onclick=async()=>{
  ...
  const subject=$('spTenderSubject')?.value.trim();const message=$('spTenderMessage')?.value.trim();if(!subject||!message){toast('Add an email subject and message');return}
